...
div(class="h-screen")
  boot-scene(
    v-if="gameStateStore.currentScene === Scenes.Boot",
    @done="gameStateStore.currentScene = Scenes.Home"
  )
...
import { currentSaveVersion, Scenes, useGameStateStore } from './stores/gameStateStore'
...
if (gameStateStore.profile.saveVersion < currentSaveVersion) {
  ...
  console.warn(
    `Resetting game state because save version is old. ${gameStateStore.profile.saveVersion} < ${currentSaveVersion}`,
  )
